#!/usr/bin/env python3
"""Ingest per-market trade census for the CLOB-era (season>=2024) resolved,
volume>0 market universe (30,289 markets per reports/01_lake_markets.md).
...
data/lake/raw/trades/<season>/<condition_id>.jsonl - written incrementally
as each page/slice comes back, so a killed run keeps whatever it fetched
(build_lake.py dedupes on trade_id, so a resumed market that re-fetches from
offset 0 just produces harmless duplicate lines). A market's condition_id is
only added to IngestState.completed_ids after its FULL pagination succeeds,
so resuming only re-does incomplete markets, never already-finished ones.
...
Usage:
    .venv/bin/python3 analysis/ingest/ingest_trades.py --seasons 2024,2025 --limit-markets 20
    .venv/bin/python3 analysis/ingest/ingest_trades.py                 # full sweep (30,289 markets)
    .venv/bin/python3 analysis/ingest/ingest_trades.py --status
"""
...
def _fetch_intel_window(cid: str, season: int, api_key: str, start_ts: int | None, end_ts: int | None) -> tuple[int, bool]:
    ...
    while True:
        ...
        if results:
            append_jsonl(JOB, f"{season}/{cid}", results)
            n_rows += len(results)
...
def fetch_market_trades_intel(cid: str, season: int, start_date: str | None, end_date: str | None, api_key: str) -> int:
    ...
    n_rows, hit_cap = _fetch_intel_window(cid, season, api_key, None, None)
    if not hit_cap:
        return n_rows
    ...
    for day_start, day_end in _daily_buckets(start, end):
        day_n, day_hit_cap = _fetch_intel_window(cid, season, api_key, int(day_start.timestamp()), int(day_end.timestamp()))
        sliced_rows += day_n
        if not day_hit_cap:
            continue
        ...
        for hr_start, hr_end in _hourly_buckets(day_start, day_end):
            hr_n, hr_hit_cap = _fetch_intel_window(cid, season, api_key, int(hr_start.timestamp()), int(hr_end.timestamp()))
            sliced_rows += hr_n
            if hr_hit_cap:
...
def fetch_market_trades_data_api(cid: str, season: int) -> int:
    ...
    while True:
        ...
        if not isinstance(page, list) or not page:
            break
        append_jsonl(JOB, f"{season}/{cid}", page)
        n_rows += len(page)
...
def process_market(row: dict) -> dict:
    ...
    for attempt in range(MAX_CONSECUTIVE_INTEL_FAILURES):
        try:
            n_rows = fetch_market_trades_intel(cid, season, start_date, end_date, api_key)
            ok = True
            break
    ...
    if not ok:
        source = "data_api"
        try:
            n_rows = fetch_market_trades_data_api(cid, season)
            ok = True
            error = None
        except IntelFailure as e:
            error = str(e)
...
def print_status() -> None:
    ...
    print(f"trades ingest status: {completed}/{total} markets complete ({100.0 * completed / total:.1f}%)")
